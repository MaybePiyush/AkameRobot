...
from AkameRobot import Piyush, dispatcher
from AkameRobot import telethn as bot
from AkameRobot.modules.connection import connected
from AkameRobot.modules.disable import DisableAbleCommandHandler
from AkameRobot.modules.helper_funcs.alternate import typing_action
from AkameRobot.modules.helper_funcs.chat_status import (
    ADMIN_CACHE,
    bot_admin,
    can_pin,
    can_promote,
...
from AkameRobot.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from AkameRobot.modules.log_channel import loggable
...
"""        
@Piyush.on_message(filters.command(["staff", "admins", "adminlist"]) & filters.group)
...
@Piyush.on_message(filters.command(["adminlist", "staff", "admins"]))
async def admins(client, message):
    try:
        adminList = []
        ownerList = []
        async for admin in Piyush.get_chat_members(
        ...
        if len(adminList) == 0:
            ...
            await Piyush.send_message(message.chat.id, text2)
        else:
...
@Piyush.on_message(filters.command("bots"))
async def bots(client, message):
    try:
        botList = []
        async for bot in Piyush.get_chat_members(
        ...
        while len(botList) > 1:
            ...
            await Piyush.send_message(message.chat.id, text3)
    except FloodWait as e:
        await asyncio.sleep(e.value)
